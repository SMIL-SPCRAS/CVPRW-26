...
def _infer_feature_dim(features_root: Path, split: str = "train") -> int:
...
def _build_loader(index_df: pd.DataFrame, cfg: TrainConfig, split: str, shuffle: bool) -> DataLoader:
    dataset = VisualDynamicDataset(
        df=index_df,
...
def train_one_epoch(
    ...
    for batch in tqdm(loader, desc="Train", leave=False):
        ...
        mask = frame_mask & valid_mask & (~missing_mask)

        optimizer.zero_grad(set_to_none=True)
        with torch.amp.autocast("cuda", enabled=amp_enabled):
            preds = model(features, mask=frame_mask)
...
def eval_one_epoch(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Val", leave=False):
            ...
            frame_mask = _frame_mask(lengths, features.shape[1], device)
            # Same logic as original best run: val on all annotated frames.
...
def run_test_export(cfg: TrainConfig, checkpoint_path: Path, run_dir: Path) -> Tuple[Path, Path]:
    ...
    test_loader = _build_loader(test_index, cfg, split="test", shuffle=False)
    ...
    txt_dir = _save_annotation_style_predictions(
        index_df=test_index,
        trialwise_pred=trialwise_pred,
        out_root=run_dir / "test_predictions_txt",
        set_name="Test_Set",
    )
    return flat_path, txt_dir
...
def load_config(path: Path) -> TrainConfig:
    ...
    for k, v in data.items():
        if not hasattr(cfg, k):
            continue
        if k in ("annotations_root", "frames_root", "test_list_path", "features_root", "output_dir", "cache_dir"):
            continue
        setattr(cfg, k, v)
    return cfg
...
def run_train(cfg: TrainConfig) -> Path:
    ...
    train_loader = _build_loader(train_index, cfg, split="train", shuffle=True)
    val_loader = _build_loader(val_index, cfg, split="val", shuffle=False)
    ...
    if bool(cfg.run_test_after_train):
        if best_ckpt_path.is_file():
            try:
                test_flat_path, test_txt_dir = run_test_export(cfg=cfg, checkpoint_path=best_ckpt_path, run_dir=run_dir)
                print(f"[DONE] Test predictions saved: {test_flat_path}")
                print(f"[DONE] Test annotation-style txt saved: {test_txt_dir}")
